...
//线性表的顺序存储结构
typedef int seqListEle;
struct seqList
{
	seqListEle* element = nullptr;
	int length = 0;
	int size = 10;
};
void test_seqList();
...
//线性表的顺序存储结构.初始化
bool seqList_init(seqList*& sL, int length)
{
	sL = new seqList;
	if (sL == nullptr)
	{
	...
	sL->element = new seqListEle[length];
	sL->length = 0;
	sL->size = length;
	return true;
}
//线性表的顺序存储结构.销毁
void seqList_destroy(seqList*& sL)
{
	if (sL == nullptr)
	{
		return;
...
//线性表的顺序存储结构.随机读取
seqListEle seqList_getEle(seqList*& sL, int pos)
{
	try
	{
		seqList_exist(sL);
...
//线性表的顺序存储结构.查找元素
int seqList_find_if(seqList*& sL, seqListEle ele, bool (*compare)(seqListEle a, seqListEle b))
{
	try
	{
		seqList_exist(sL);
...
//线性表的顺序存储结构.插入元素
bool seqList_insert(seqList*& sL, int pos, seqListEle ele)
{
	try
	{
		seqList_exist(sL);
...
//线性表的顺序存储.删除元素
bool seqList_delete(seqList*& sL, int pos)
{
	try
	{
		seqList_exist(sL);
...
//线性表的顺序存储.遍历线性表
void seqList_traverse(seqList*& sL)
{
	try
	{
		seqList_exist(sL);
...
//线性表的顺序存储.扩容
bool seqList_expand(seqList*& sL, double times)
{
	int n = sL->length;
	seqListEle* arr = new seqListEle[n];
	if (arr == nullptr)
	{
		return false;
	}
	...
	delete[]sL->element;
	sL->element = new seqListEle[n * times];
	for (int i = 0; i < n; i++)
	{
		sL->element[i] = arr[i];
	}
...
//线性表的顺序存储.测试
void test_seqList(void)
{
	seqList* sList = nullptr;
	seqList_init(sList, 5);
	for (int i = 0; i < 5; i++)
	{
		seqList_insert(sList, sList->length, i);
	}
	std::cout << "初始化后的线性表长度为" << seqList_length(sList) << std::endl;
	std::cout << "遍历线性表：" << std::endl;
	seqList_traverse(sList);
	std::cout << "最后一个元素是" << sList->element[sList->length - 1] << std::endl;
	seqList_delete(sList, sList->length - 1);
	std::cout << "删除最后一个元素后的线性表长度为" << seqList_length(sList) << std::endl;
	std::cout << "最后一个元素是" << sList->element[sList->length - 1] << std::endl;
	auto function = [](seqListEle x, seqListEle y) {return y == x; };
	int value = 0;
	std::cout << "输入要查找的值：";
	std::cin >> value;
	std::cout << seqList_find_if(sList, value, function) << std::endl;
	seqList_clear(sList);
	std::cout << "清空线性表后的长度为" << seqList_length(sList) << std::endl;
	std::cout << "线性表的容量是" << sList->size << std::endl;
	seqList_expand(sList, 12.5);
	std::cout << "扩容后的容量是" << sList->size << std::endl;
	seqList_destroy(sList);
	std::cout << "线性表的顺序存储测试完成！！！" << std::endl << std::endl;
	return;
}

//线性表的链式存储.初始化
bool linkedList_init(linkedListNode*& lL)
{
...
void test_linkedList()
{
	...
	std::cout << "线性表的链式存储测试完成！！！" << std::endl << std::endl;
	return;
}
...
//静态链表.测试
void test_staticallyLinkedList(void)
{
	sLL sLList;
	...
	staticallyLinkedList_traverse(sLList);
	std::cout << std::endl << "静态链表测试完成！！！" << std::endl << std::endl;
}
